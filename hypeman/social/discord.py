...
class DiscordPlatform(SocialPlatform):
    ...
    def __init__(self, default_event_kind: str = EVENT_LIVE, **credentials):
        """
        Args:
            default_event_kind: What to assume an announcement is about when the
                caller doesn't say. Boon-Tube passes EVENT_UPLOAD; stream-daemon
                takes the EVENT_LIVE default. Per-call values in stream_data
                always win over this.
            **credentials: Optional credential overrides (webhook_url, role_id).
        """
    ...
    def authenticate(self):
        ...
        self.webhook_url = platform_secret('Discord', 'webhook_url')
        
        # Get per-platform webhook URLs (optional - overrides default)
        for platform in ['youtube', 'tiktok', 'twitch', 'kick']:
            platform_webhook = platform_secret('Discord', f'webhook_{platform}')
            if platform_webhook:
        ...
        self.role_id = platform_secret('Discord', 'role')
        
        # Get per-platform role IDs (optional - overrides default)
        for platform in ['youtube', 'tiktok', 'twitch', 'kick']:
            platform_role = platform_secret('Discord', f'role_{platform}')
            if platform_role:
    ...
    def post(self, message: str, reply_to_id: Optional[str] = None, platform_name: Optional[str] = None, stream_data: Optional[dict] = None) -> Optional[str]:
        ...
        try:
            ...
            if first_url and stream_data:
                ...
                elif is_url_for_domain(first_url, 'tiktok.com'):
            ...
            logger.error(f"✗ Discord post failed: {type(e).__name__}: {e}")
            return None
    ...
    def update_stream(self, platform_name: str, stream_data: dict, stream_url: str) -> bool:
        ...
        try:
            ...
            if is_url_for_domain(stream_url, 'twitch.tv') or platform_key == 'twitch':
                color = 0x9146FF
                platform_title = "🟣 Live on Twitch"
            elif is_url_for_domain(stream_url, 'youtube.com') or is_url_for_domain(stream_url, 'youtu.be') or platform_key == 'youtube':
                color = 0xFF0000
                platform_title = "🔴 Live on YouTube"
            elif is_url_for_domain(stream_url, 'kick.com') or platform_key == 'kick':
                color = 0x53FC18
                platform_title = "🟢 Live on Kick"
            ...
            logger.error(f"✗ Discord update failed: {type(e).__name__}: {e}")
            return False
    ...
    def end_stream(self, platform_name: str, stream_data: dict, stream_url: str) -> bool:
        ...
        try:
            ...
            logger.error(f"✗ Discord stream ended update failed: {type(e).__name__}: {e}")
            return False
